...
def plot_multinomial(
    dist: rv_discrete_frozen,
    title: str | None = None,
    ax_labels: tuple | None = None,
):
    ...
    shading = ax.tripcolor(
        t, l, r, pmf, cmap='turbo', shading='flat', rasterized=True
    )
    contour = ax.tricontour(
        t, l, r, pmf, colors='k', alpha=0.5, levels=3, linewidths=0.5
    )
    ...
    ax.scatter(
        ...
        r,
        color='skyblue',
        label=f'$\\mu$: {t:0.2g}, {l:0.2g}, {r:0.2g}',
        marker='x',
    )
    ...
    ax.grid(alpha=0.1)
    ax.set_title(
        rf'$\mathbf{{p}} = {dist.p[0]:0.2g}, {dist.p[1]:0.2g}, {dist.p[2]:0.2g}$',
        fontsize=9,
    )
    if title:
...
def plot_scored_pdf(dist: fd.Scored, title: str | None = None):
    ...
    shading = ax.tripcolor(
        t, l, r, v, cmap='turbo', shading='gouraud', rasterized=True
    )

    contour = ax.tricontour(
        t, l, r, v, colors='k', alpha=0.3, levels=3, linewidths=0.5
    )
    ax.clabel(contour, fontsize=8)
    t, l, r = dist.mode
    ax.scatter(
        t, l, r, color='aquamarine', label=f'mode: {dist.mo:0.2f}', marker='.'
    )
    t, l, r = dist.mean
    ax.scatter(
        t, l, r, color='skyblue', label=f'$\\mu$: {dist.mu:0.2f}', marker='x'
    )
