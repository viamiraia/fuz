    ...
    mo.callout(
        mo.vstack(
            [
                ...
                mo.hstack(
                    [mo.md('$r$ axis'), w_ternr], widths=[1, 5], align='center'
                ),
                mo.hstack(
                    [mo.md('$t$ axis'), w_ternt], widths=[1, 5], align='center'
                ),
            ]
        ),
        kind='info',
...
@app.cell(hide_code=True)
def _(mo):
    w_infval = mo.ui.slider(
        0.01, 0.99, 0.01, 0.01, full_width=True, show_value=True
    )
    ...
    mo.callout(
        mo.vstack(
            [
                ...
                mo.hstack(
                    [mo.md('score'), w_infval], widths=[1, 5], align='center'
                ),
                mo.hstack(
                    [mo.md('count'), w_infcnt], widths=[1, 5], align='center'
                ),
            ]
        ),
        kind='warn',
...
def _(alt, get_like_infdir, mo, np, pd, w_infcnt, w_infval):
    ...
    _chart = (
        alt.Chart(infdf1)
        .mark_line()
        .encode(alt.X('dimensions'), alt.Y('mu').scale(zero=False))
    )
    mo.ui.altair_chart(_chart)
...
def _(alt, infdf1, mo):
    _chart = (
        alt.Chart(infdf1)
        .mark_line()
        .encode(alt.X('dimensions'), alt.Y('y').scale(zero=False))
    )
    mo.ui.altair_chart(_chart)
