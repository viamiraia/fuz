...
def _(mo):
    wintro_no = mo.ui.slider(
        ...
        value=2,
        show_value=True,
        label='no. of no/tails/downvotes',
        full_width=True,
    )
    wintro_yes = mo.ui.slider(
        ...
        value=6,
        show_value=True,
        label='no. of yes/heads/upvotes',
        full_width=True,
    )
    mo.callout(
        mo.hstack([wintro_no, wintro_yes], widths='equal', align='center'),
        kind='info',
    )
...
def _(
    ...
    _noulli_chart = plot_bernoulli(intro2_p).properties(
        title='bernoulli pmf', width=100
    )
...
def _(Callable, Sequence, alt, np, pd):
    def plot_betas(
        x: np.ndarray, pdfs: Sequence[Callable], names: Sequence[str]
    ) -> alt.Chart:
        dfs = []
...
def _(
    ...
    _beta_fig = plot_betas(
        x_small,
        (b_mo_t.pdf, b_mo_k1.pdf, b_mu_k1.pdf),
        ('β mode trials', 'β mode k', 'β μ k'),
    )
...
def _(Sequence, alt, np, pd):
    def plot_bernoulli(p: Sequence[float, float] | np.ndarray) -> alt.Chart:
        ...
        _base = alt.Chart(
            _df, title='bernoulli probability mass function (pmf)'
        ).encode(
            alt.X('x', axis=alt.Axis(labelAngle=0)),
            alt.Y('probability', scale=alt.Scale(domain=[0, 1])),
        )
        _bar = _base.mark_bar()
        _text = _base.mark_text(dy=-10).encode(
