...
get_norm_const.__doc__ = (
    'Gets pooling normalizing constant for continuous belief RVs.'
)
...
def _geom_pool(
    bounds: tuple[float, float], x: ft.NPVec, rvs: ft.ContDists
) -> ft.NPVec:
...
def num_f_weighted_s(
    pdf_s: Sequence[ft.DistVec], weights: ft.NPVec
) -> ft.PoolNumFS:
...
def pool_samples_f(num_f_s: ft.PoolNumF) -> ft.PoolNumFS:
...
def pool(  # type: ignore[reportGeneralTypeIssues]
    ...
    if weights is not None:
        return weighted_pool(x_arr, rvs, weights)
    if method == 'mult':
        return mult_pool(x_arr, rvs)
    if method == 'geom':
        return geom_pool(x_arr, rvs)
...
def pool(
    ...
    x_arr = np.asarray([x], dtype=float)
    if weights is not None:
        return weighted_pool_s(x_arr, rvs, weights)
    if method == 'mult':
        return mult_pool_s(x_arr, rvs)
    if method == 'geom':
        return geom_pool_s(x_arr, rvs)
...
def holder_pool_s(
    ...
    if weights is not None:
        num = np.sum(
            [
                w * sample**alpha
                for sample, w in zip(pdf_samples, weights, strict=True)
            ],
            axis=0,
        ) ** (1 / alpha)
    else:
        num = np.sum([sample**alpha for sample in pdf_samples], axis=0) ** (
            1 / alpha
        )
