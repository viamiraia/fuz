...
async function getWorkflowNodes(plugin, canvas, selectedNodeID) {
  ...
  if (selectedNodeData === void 0)
    return;
  const isSettingsNode = await checkContainsLanguage(plugin.app, selectedNodeData, canvasBlockSettingsLanguageName);
...
var FuzzyScriptSuggester = class extends import_obsidian4.FuzzySuggestModal {
  ...
  getItems() {
    const files = [];
    const folder = app.vault.getAbstractFileByPath((0, import_obsidian4.normalizePath)(this.plugin.settings.workflowScriptFolder));
    if (folder === null)
      return [];
  ...
  onChooseItem(item) {
    const view = this.plugin.app.workspace.getActiveViewOfType(import_obsidian4.ItemView);
    if (view === null)
      return;
    if (!view.hasOwnProperty("canvas")) {
      new import_obsidian4.Notice("A canvas must be open to run this command");
      return;
    }
    const canvas = view.canvas;
    const tx = canvas.tx;
...
var CanvasBlocksPlugin2 = class extends import_obsidian4.Plugin {
  ...
  async onload() {
    ...
    this.registerEvent(this.app.workspace.on("layout-change", () => {
      ...
      const canvas = view.canvas;
      if (view.originalAddEdge)
        canvas.addEdge = view.originalAddEdge;
      if (view.originalAddNode)
        canvas.addNode = view.originalAddNode;
      if (view.originalRemoveNode)
        canvas.removeNode = view.originalRemoveNode;
      if (view.originalMenuRender)
        canvas.menu.render = view.originalMenuRender;
      if (view.originalHandlePaste)
        canvas.handlePaste = view.originalHandlePaste;
      view.originalHandlePaste = canvas.handlePaste;
      canvas.handlePaste = async function(event) {
        var _a;
        if (!view.originalHandlePaste)
          return;
        ...
        for (const node of data.nodes) {
          if (node.type === "file") {
            ...
            const file = that.app.vault.getAbstractFileByPath((0, import_obsidian4.normalizePath)(node.file));
            if (!(file instanceof import_obsidian4.TFile)) {
              newNodes.push(node);
              continue;
            }
        ...
        for (const settingsNode of settingsNodes) {
          const file = that.app.vault.getAbstractFileByPath((0, import_obsidian4.normalizePath)(settingsNode.file));
          if (!(file instanceof import_obsidian4.TFile))
            continue;
      ...
      canvas.menu.render = async function(redraw) {
        if (!view.originalMenuRender)
          return;
        ...
        (0, import_obsidian4.setTooltip)(button, "Execute script");
      ...
      canvas.addEdge = async function(edge) {
        ...
        if (edge.originalEdgeUpdate)
          edge.update = edge.originalEdgeUpdate;
      ...
      canvas.addNode = async function(node) {
        if (!view.originalAddNode)
          return;
        if (!node.skipAddNode) {
        ...
        if (node.originalMoveAndResize === void 0) {
          node.originalMoveAndResize = node.moveAndResize;
          node.moveAndResize = async function(newPositionAndSize) {
            if (!node.originalMoveAndResize)
              return;
            canvas.requestSave();
      ...
      canvas.removeNode = async function(deletedNode) {
        if (!view.originalRemoveNode)
          return;
        canvas.requestSave();
